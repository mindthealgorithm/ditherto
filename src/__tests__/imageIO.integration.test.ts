...
describe('Complete dithering pipeline integration tests', () => {
  ...
  function resultToImageData(
    ...
    if (!isUint8ArrayLike(result)) throw new Error('Result is neither ImageData nor Uint8Array');
    const rgbData = result as Uint8Array;

    const pixelCount = rgbData.length / 3;
    const width =
      expectedWidth ?? (expectedHeight ? pixelCount / expectedHeight : Math.sqrt(pixelCount));
    const height = expectedHeight ?? pixelCount / width;
    if (!Number.isInteger(width) || !Number.isInteger(height) || width * height !== pixelCount) {
  ...
  function checkGameBoyPalette(colors: Map<string, number>): boolean {
    const gameboyColors = ['15,56,15', '48,98,48', '139,172,15', '155,188,15'];
